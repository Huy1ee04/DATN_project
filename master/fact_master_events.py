#!/usr/bin/env python3
"""
fact_master_events.py (master) — Resolve surrogate keys.

Đọc fact_stock_events từ stage_2, resolve FK chuẩn Kimball.
...
Destination (MinIO):
  master/fact/fact_master_events.parquet
...
Output schema:
  ...
  event_name_vi   (Utf8)  — measure
  event_title_vi  (Utf8)  — measure
  event_code      (Utf8)  — measure
"""
...
logger = logging.getLogger("fact_master_events_publish")
...
SRC_FACT_PATH = "transformed/stage_2/fact/fact_stock_events.parquet"
SRC_DIM_STOCK_PATH = "master/dimension/dim_master_stock.parquet"
DST_PREFIX = "master/fact"
DST_FILENAME = "fact_master_events.parquet"
...
def resolve_fk(df_fact, df_dim_stock):
    ...
    # public_date_key = yyyymmdd (Role-Playing Dimension: ngày công bố)
...
def parse_args():
    p = argparse.ArgumentParser(description="Master: fact_stock_events with FK.")
...
def main():
    ...
    logger.info(
        "\n%s\nMaster: fact_stock_events → fact_master_events (resolve FK)\n%s\n"
        "Fact source : s3://%s\nDim stock   : s3://%s\nDest        : s3://%s\nRun at      : %s\n%s",
    ...
    logger.info("\n%s\nfact_master_events publish complete!\n%s", separator, separator)
