#!/usr/bin/env python3
"""
fact_master_events.py

Đưa dữ liệu từ layer transformed lên master (fact).
...
SRC_PATH = "transformed/fact/fact_stock_events.parquet"
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Publish fact_stock_events.parquet → master/fact/fact_master_events.parquet"
    )
...
def log_run_info(args: argparse.Namespace, src: str, dst: str) -> None:
    ...
    logger.info(
        "\n%s\nPublish: fact_stock_events → master/fact/fact_master_events\n%s\n"
        "MinIO Endpoint : %s\n"
        "Bucket         : %s\n"
        "Source         : s3://%s\n"
        "Destination    : s3://%s\n"
