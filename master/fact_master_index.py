#!/usr/bin/env python3
"""
fact_master_index.py (master)

Đọc fact_market_index từ stage_2, resolve surrogate keys chuẩn Kimball.
...
Destination (MinIO):
  master/fact/fact_master_index.parquet

Logic:
  1. Đọc stage_2 fact_market_index.
  2. JOIN dim_master_index: symbol (OHLCV) = index_symbol (dim) → lấy index_key.
...
logger = logging.getLogger("fact_master_index_publish")
...
DST_FILENAME = "fact_master_index.parquet"
...
def resolve_surrogate_keys(
    ...
    - symbol (OHLCV) = index_symbol (dim) → index_key  (JOIN dim_master_index)
    ...
    # dim_master_index có index_symbol (= "VNINDEX", "VN30", ...)
    # fact_market_index có symbol (= "VNINDEX", "VN30", ...)
    ...
    other_cols = [c for c in df_fact.columns if c not in fk_cols + nk_cols]
    output_order = fk_cols + other_cols
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Master: publish fact_market_index with surrogate keys (index_key, date_key)."
    )
...
def main() -> None:
    ...
    logger.info(
        "\n%s\nMaster: fact_market_index → fact_master_index (resolve FK)\n%s\n"
        "MinIO Endpoint : %s\n"
        "Fact source    : s3://%s\n"
        "Dim index      : s3://%s\n"
        "Destination    : s3://%s\n"
    ...
    logger.info("\n%s\nfact_master_index publish complete!\n%s", separator, separator)
