#!/usr/bin/env python3
"""
fact_market_index_daily.py (master publish)

Đưa dữ liệu daily index từ transformed lên master, giữ nguyên cấu trúc thư mục partition.

Source (MinIO):
  transformed/fact/market/index_daily/year=YYYY/month=MM/day=DD/ohlc.parquet

Destination (MinIO):
  master/fact/market/index_daily/year=YYYY/month=MM/day=DD/ohlc.parquet
...
logger = logging.getLogger("fact_market_index_daily_master")
...
SRC_PREFIX = "transformed/fact/market/index_daily"
DST_PREFIX = "master/fact/market/index_daily"
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description=(
            "Publish transformed/fact/market/index_daily → master/fact/market/index_daily "
...
def log_run_info(args: argparse.Namespace, src_root: str, updated_at: str) -> None:
    ...
    logger.info(
        "\n%s\nPublish: transformed/.../index_daily → master/.../index_daily\n%s\n"
        "MinIO Endpoint : %s\n"
        "Bucket         : %s\n"
        "Source         : s3://%s/\n"
        "Destination    : s3://%s/%s/\n"
...
def main() -> None:
    ...
    separator = "=" * 80
    logger.info("\n%s\nfact_market_index_daily publish complete! %d file(s) written, %d skipped.\n%s",
