#!/usr/bin/env python3
"""
fact_master_stock_signals.py (master)

Đọc fact_stock_signals từ stage_2, resolve surrogate keys từ dimension tables.
...
Destination (MinIO):
  master/fact/fact_master_stock_signals.parquet

Logic:
  1. Đọc stage_2 fact_stock_signals (đã gán nhãn)
  ...
  5. Giữ natural keys (symbol, trade_date) cho audit
"""
...
logger = logging.getLogger("fact_master_stock_signals_publish")
...
SRC_FACT_PATH = "transformed/stage_2/fact/fact_stock_signals.parquet"
SRC_DIM_STOCK_PATH = "master/dimension/dim_master_stock.parquet"
DST_PREFIX = "master/fact"
DST_FILENAME = "fact_master_stock_signals.parquet"
...
def resolve_surrogate_keys(
    ...
    output_order = ["stock_key", "date_key", *signal_cols]
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Master: publish fact_stock_signals with surrogate keys (stock_key, date_key)."
    )
...
def main() -> None:
    ...
    logger.info(
        "\n%s\nMaster: fact_stock_signals → fact_master_stock_signals (resolve FK)\n%s\n"
        "MinIO Endpoint  : %s\n"
        "Fact source     : s3://%s\n"
        "Dim stock       : s3://%s\n"
        "Destination     : s3://%s\n"
    ...
    logger.info("\n%s\nfact_master_stock_signals publish complete!\n%s", separator, separator)
