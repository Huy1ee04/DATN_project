...
SRC_PATH = "transformed/dimension/dim_stock_info.parquet"
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Publish dim_stock_info.parquet → master/dimension/dim_master_stock.parquet"
    )
...
def log_run_info(args: argparse.Namespace, src: str, dst: str) -> None:
    ...
    logger.info(
        "\n%s\nPublish: dim_stock_info → master/dimension/dim_master_stock\n%s\n"
        "MinIO Endpoint : %s\n"
        "Bucket         : %s\n"
        "Source         : s3://%s\n"
        "Destination    : s3://%s\n"
