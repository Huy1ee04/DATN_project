...
SRC_PATH = "transformed/fact/fact_stock_news.parquet"
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Publish fact_stock_news.parquet → master/fact/fact_master_news.parquet"
    )
...
def log_run_info(args: argparse.Namespace, src: str, dst: str) -> None:
    ...
    logger.info(
        "\n%s\nPublish: fact_stock_news → master/fact/fact_master_news\n%s\n"
        "MinIO Endpoint : %s\n"
        "Bucket         : %s\n"
        "Source         : s3://%s\n"
        "Destination    : s3://%s\n"
