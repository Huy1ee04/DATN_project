#!/usr/bin/env python3
"""
dim_index_info (stage_2) — passthrough / conforming layer.
...
logger = logging.getLogger("dim_index_info_stage2")
...
SRC_PATH     = "transformed/stage_1/dimension/dim_index_info.parquet"
DST_PREFIX   = "transformed/stage_2/dimension"
DST_FILENAME = "dim_index_info.parquet"

# Schema cần đảm bảo từ stage_1
REQUIRED_COLS = {"index_id", "index_symbol", "index_name", "description", "group"}
...
def validate_schema(df: pl.DataFrame) -> None:
    ...
    if missing:
        raise ValueError(
            f"stage_1 dim_index_info thiếu cột: {sorted(missing)}. "
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description=(
            "Stage 2: passthrough dim_index_info từ stage_1 → stage_2 "
...
def log_run_info(args: argparse.Namespace, src: str, dst: str) -> None:
    ...
    logger.info(
        "\n%s\nStage 2: dim_index_info (passthrough + validate)\n%s\n"
        "MinIO Endpoint : %s\n"
        "Bucket         : %s\n"
        "Source         : s3://%s\n"
        "Destination    : s3://%s\n"
...
def main() -> None:
    ...
    gx_check_columns_not_null(df, {"columns": ["index_id", "index_symbol"]})
    gx_check_column_values_unique(df, {"column": "index_id"})
    ...
    separator = "=" * 80
    logger.info("\n%s\ndim_index_info stage_2 complete!\n%s", separator, separator)
