#!/usr/bin/env python3
"""
fact_stock_news.py (stage_2) — Validate + Passthrough.

Đọc fact_stock_news từ stage_1, validate schema, ghi nguyên xi sang stage_2.
...
logger = logging.getLogger("fact_stock_news_stage2")
...
SRC_PATH = "transformed/stage_1/fact/fact_stock_news.parquet"
DST_PREFIX = "transformed/stage_2/fact"
DST_FILENAME = "fact_stock_news.parquet"

REQUIRED_COLS = {"symbol", "public_date", "news_title"}
...
def parse_args():
    p = argparse.ArgumentParser(description="Stage 2: validate fact_stock_news.")
...
def main():
    ...
    separator = "=" * 80
    logger.info("\n%s\nStage 2: validate fact_stock_news\n%s\nSource: s3://%s\nDest:   s3://%s\n%s",
    ...
    gx_check_columns_not_null(df, {"columns": ["symbol", "public_date", "news_title"]})
    ...
    logger.info("\n%s\nfact_stock_news stage_2 complete!\n%s", separator, separator)
