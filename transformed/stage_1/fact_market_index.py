...
Output (MinIO):
  transformed/stage_1/fact/fact_market_index.parquet   (single consolidated file)
...
logger = logging.getLogger("fact_market_index_transform")
...
def read_partition(fs: s3fs.S3FileSystem, bucket: str, prefix: str) -> pl.DataFrame:
    ...
    # Chỉ lấy ohlc.parquet — tránh lẫn summary.parquet schema khác
...
def log_run_info(args: argparse.Namespace, source_prefix: str, dst_path: str) -> None:
    ...
    logger.info(
        "\n%s\nTransform: raw index intraday → fact index daily (single file)\n%s\n"
        "MinIO Endpoint : %s\n"
        "Bucket         : %s\n"
        "Source         : s3://%s/%s/\n"
        "Destination    : s3://%s\n"
...
def main() -> None:
    ...
    # ── GX Gate ─────────────────────────────────────────────────────────
    logger.info("Running GX validation (Stage 1)...")
    gx_check_columns_not_null(df_daily, {"columns": ["symbol", "trade_date"]})
    gx_check_compound_columns_unique(df_daily, {"column_list": ["symbol", "trade_date"]})
    ...
    separator = "=" * 80
    logger.info("\n%s\nfact_market_index transform complete!\n%s", separator, separator)
